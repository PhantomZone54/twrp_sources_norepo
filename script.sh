...
if [ $DDF -gt 8192 ]; then
  ...
  time tar -I pxz --memlimit-compress='70%' -T2 -cvf - * | split -b 1792M - ~/project/files/$RecName-$BRANCH-norepo-$(date +%Y%m%d).tar.xz.
  ...
  time tar -I pxz --memlimit-compress='70%' -T2 -cvf ~/project/files/$RecName-$BRANCH-norepo-$(date +%Y%m%d).tar.xz *
