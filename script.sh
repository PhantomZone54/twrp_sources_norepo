...
if [ $DDF -gt 8192 ]; then
  ...
  tar -I pxz -T2 -cvf - * | split -b 1792M - ~/project/files/$RecName-$BRANCH-norepo-$(date +%Y%m%d).tar.xz.
  ...
  tar -I pxz -T2 -cvf ~/project/files/$RecName-$BRANCH-norepo-$(date +%Y%m%d).tar.xz *
